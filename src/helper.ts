...
import { getRect, is, removeLayer } from "./api/api";
...
export function convertUnit(length, isText, percentageType) {
    if (length.length) {
        var units = /*this.*/context.configs.unit.split("/"),
            unit = units[0];
        ...
        let len = Math.round(length / /*this.*/context.configs.scale * 10) / 10,
            units: string[] = /*this.*/context.configs.unit.split("/"),
            unit = units[0];
...
export function toggleHidden() {
    var isHidden = (/*this.*/context.configs.isHidden) ? false : !Boolean(/*this.*/context.configs.isHidden);
    /*this.*/context.configs = /*this.*/setConfigs({
        isHidden: isHidden
    });
...
export function toggleLocked() {
    var isLocked = (/*this.*/context.configs.isLocked) ? false : !Boolean(/*this.*/context.configs.isLocked);
    /*this.*/context.configs = /*this.*/setConfigs({
        isLocked: isLocked
    });
